...
const App = () => {
  ...
  const resetAndCreateGrid = () => {
    ...
    let tmGrid: GridItemType[] = [];
    for(let i = 0; i < (items.length * 2); i++) tmGrid.push({
  ...
  const handleItemClick = (index: number) => {
    if(playing && index !== null && shownCount < 2) {
      let tmGrid = [...gridItems];
      if(tmGrid[index].permanentShown === false && tmGrid[index].shown === false) {
        tmGrid[index].shown = true;
